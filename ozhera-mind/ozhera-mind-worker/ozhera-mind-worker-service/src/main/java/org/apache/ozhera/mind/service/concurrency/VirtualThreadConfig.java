 ...
 * Enables virtual threads for high concurrency with low memory footprint.
 */
@Slf4j
@Configuration
public class VirtualThreadConfig {
    ...
    public TomcatProtocolHandlerCustomizer<?> virtualThreadExecutorCustomizer() {
        return protocolHandler -> {
            ...
            log.info("Tomcat configured to use virtual threads");
        };
    }

    /**
     * Virtual thread executor for Agent tasks.
     */
    @Bean
    public ExecutorService agentVirtualExecutor() {
        log.info("Created virtual thread executor for agent tasks");
